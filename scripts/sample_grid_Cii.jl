#=
Script to sample the covariance matrix C_ii over a grid of r = r' and cosθ = 1 values.
The underlying cosmological choice is banked in through the power spectrum interpolator.
The script saves the computed covariance matrix elements.
=#
using PecVelCov
using ProgressMeter
using Base.Threads
...
rs = LinRange(0.1, 350, 50000)  # Mpc / h
ks = 10 .^ LinRange(-4, 1, 2 * 2048)
println("Sampling diagonal covariance matrix elements over $(length(rs)) grid points.")
...
djn_interp = precompute_djn(ell_min, ell_max, krs);
start_krs = precompute_djn_start(djn_interp, krs)

nr = length(rs)
Cii_grid = zeros(nr)
# The Legendre polynomials for cosθ = 1, i.e. all are just 1.
